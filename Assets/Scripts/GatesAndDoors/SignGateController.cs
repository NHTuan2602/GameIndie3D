...
public class SignGateController : MonoBehaviour
{
    ...
    public Material safeRoadMaterial;
    public Material dangerRoadMaterial;
    ...
    // ==========================================
    // HÀM XỬ LÝ CHIA LÀN CHÍNH XÁC THEO LUẬT
    // ==========================================
    public void SetupGateFor4Lanes(bool willBeReversed)
    {
        for (int i = 0; i < 4; i++)
        {
            bool isSafeLane;

            // Nếu phía trước là đường Đảo Chiều -> 2 làn bên TRÁI (0, 1) là đường Sống
            // Nếu phía trước là đường Bình Thường -> 2 làn bên PHẢI (2, 3) là đường Sống
            if (willBeReversed) isSafeLane = (i == 0 || i == 1);
            else isSafeLane = (i == 2 || i == 3);

            if (isSafeLane)
            {
                // LÀN ĐƯỢC PHÉP ĐI (MŨI TÊN XANH)
                if (signBoards[i] != null) signBoards[i].material = safeMaterial;
                if (deathBlockers[i] != null) deathBlockers[i].SetActive(false);
                if (roadDecals[i] != null && safeRoadMaterial != null) roadDecals[i].material = safeRoadMaterial;
            }
            else
            {
                // LÀN BỊ CẤM (TRẢ LẠI RANDOM CHO BIỂN TRÊN CAO!)
                ...
                // Dưới đất thì bắt buộc phải là DẤU X ĐỎ
                if (roadDecals[i] != null && dangerRoadMaterial != null) roadDecals[i].material = dangerRoadMaterial;
            }
        }
    }
}